# Information Security Course Material (ITF22)

This repository contains **Information Security course material** that I studied in my 7th semester as part of my BSIT degree.

The purpose of creating this repository is:
- 📚 To help **junior students** easily access study material  
- 🔐 To maintain a **personal backup** of important course resources  
- 🤝 To share knowledge for **educational purposes only**

---

## 📂 Repository Structure

### 🔹 Activities
Contains practice activities and command references related to Information Security topics.
- Information Security Activities
- commands.txt

### 🔹 FINALS
Final-term reference material including:
- AI Security
- Network Security
- Secure Coding
- OWASP Secure Coding Guidelines
- OSI Model Cheat Sheet

### 🔹 MIDS
Mid-term syllabus material including:
- Introduction and Basics
- Cryptography
- Network Security
- SSL / TLS
- Packet Capture files (.pcap)
- Lab notes and handouts

### 🔹 Bandit
Resources related to **Bandit (OverTheWire)** practice:
- Playlist
- Activity notes

### 🔹 Quiz
- My quiz 3 solution


---

## 🎯 Who Is This Repo For?
- BSIT / CS students
- Juniors studying Information Security
- Anyone revising basic Information Security concepts

---

## ⚠️ Disclaimer
All material shared here is **strictly for educational purposes**.  
Ownership of lecture slides and PDFs belongs to the respective instructor.

---

## ⭐ Contribution
If this repository helps you, feel free to **star ⭐ it**.  
Suggestions and improvements are welcome.

---

## 👩‍💻 Maintained By
**Maheen Fatima**  
